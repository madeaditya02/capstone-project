    ...
    <div className="flex gap-4 rounded-xl bg-white px-5 py-4 shadow-md items-center">
      ...
      >
        <div className="flex items-center gap-1.5 sm:gap-4">
          <Link to={profileUrl} className="flex size-9 sm:size-12 items-center justify-center rounded-full bg-primary text-sm sm:text-base font-bold text-slate-800 shrink-0">
            {initials(friend.name)}
          </Link>
          <div className="flex md:flex-col w-full justify-between">
            <Link to={profileUrl} className="sm:text-lg font-semibold text-slate-800 hover:underline">{friend.name}</Link>
